...
namespace infer {
    ...
    namespace ops {
        ...
        void dequantize_q3_K_row(const uint8_t* packed, float* dst, int64_t n) {
            ...
            for (int b = 0; b < nb; ++b) {
                ...
                size_t blk_size = std::min((size_t)256, (size_t)(n - b * 256));
                for (int i = 0; i < 16 && i * 16 < (int)blk_size; ++i) {
        ...
        void dequantize_q4_K_row(const uint8_t* packed, float* dst, int64_t n) {
            ...
            for (int b = 0; b < nb; ++b) {
                ...
                for (int i = 0; i < 16 && i * 16 < (int)blk_size; ++i) {
                    ...
                    size_t sub_blk = std::min((size_t)16, blk_size - i * 16);
                    for (size_t j = 0; j < sub_blk; ++j) {
                        uint8_t v = (packed[j / 2] >> ((j % 2) * 4)) & 0x0F;
        ...
        void dequantize_q5_K_M_row(const uint8_t* packed, float* dst, int64_t n) {
            ...
            for (int b = 0; b < nb; ++b) {
                ...
                size_t blk_size = std::min((size_t)256, (size_t)(n - b * 256));
                for (int i = 0; i < 8 && i * 32 < (int)blk_size; ++i) {
                    uint16_t scale_f16, min_f16;
    ...
    public:
        ...
        bool load(const std::string& path) {
            ...
            std::cout << "✓ GGUF loaded: offset=" << tensor_data_start_offset_
    ...
    private:
        ...
        DataType gguf_to_dtype(uint32_t t) const {
            ...
            case 6:  return DataType::Q5_K_M;
            case 7:  return DataType::Q6_K;
            case 8:  return DataType::Q8_0;
            case 12: return DataType::Q3_K;
            case 14: return DataType::Q4_K_M;
            default:
    ...
    private:
        ...
        void gemm_weight(int M, int N, int K, const float* A, const Tensor* B, float* C) {
            ...
            case DataType::Q6_K:
                ...
                break;
            default:
                std::cerr << "✗ Unsupported dtype: " << static_cast<int>(B->dtype()) << std::endl;
    ...
    public:
        ...
        std::vector<int> encode(const std::string& s) {
            if (use_char_) {
                std::vector<int> t;
                for (char c : s) t.push_back((unsigned char)c);
                return t;
            }
            std::vector<int> t;
            std::string w;
            for (char c : s) {
                if (c == ' ') {
                    if (!w.empty()) {
                        ...
                        t.push_back(it != vocab_.end() ? it->second : 0);
                        w.clear();
                    }
                }
                else {
        ...
        std::string decode(int id) {
            ...
            return it != id2tok_.end() ? it->second : "<unk>";
        }
    private:
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cerr << "Usage: " << argv[0] << " <model.gguf> [vocab.json]\n";
        return 1;
    }
    try {
        ...
        if (argc >= 3) tok.load(argv[2]);
        else tok.load("");
        std::cout << "Prompt: ";
        std::string prompt;
        std::getline(std::cin, prompt);
        auto toks = tok.encode(prompt);
        if (toks.empty()) return 1;
        std::vector<std::unique_ptr<infer::Tensor>> kvcache;
        int pos = 0;
        for (int t : toks) model.forward(t, pos++, kvcache);
        int next = toks.back();
